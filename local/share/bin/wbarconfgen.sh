...
if [ "$reload_flag" == "1" ] ; then
    killall waybar
    # No --config here on purpose: the watchdog then falls back to the graphite
    # bar. Naming HyDE's regenerated config.jsonc is exactly what made the
    # designed bar disappear on every theme and wallpaper change.
